...
const checkPaymentTransaction = async (hash, order) => {
  if (!listener.isOpen()) {
    await listener.open().then(beginListen);
  }
  const transaction = await checkConfirmedTransaction(hash);
  let mosaic = transaction.mosaics.filter(
    (mosaic) => mosaic.id.id.toHex() === MOSAIC_ID,
  );
  ...
  const plainMessage = EncryptedMessage.decrypt(
    message,
    STORE_PRIV_KEY,
    signer,
    network,
  ...
  const isValidPayment =
    orderDetail.orderId === order.id && order.total_price === paidAmount;
...
const sendToken = async (amount, message, address, publicKey) => {
  ...
  const res = await new Promise((resolve, reject) => {
    ...
    transactionService
      ...
      .subscribe(succeeded, failured);
  }).catch((err) => ({ isError: true, error: err }));

  await new Promise((res) => setTimeout(res, 2000));
  await new Promise((resolve, reject) => {
    console.log('checking transaction');
    transactionService.getTransactionStatus(signedTransaction.hash).subscribe(
