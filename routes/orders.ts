import { getPaymentService, NemPaymentResult } from '../services/payments';
...
router.post('/update', async (req, res) => {
  ...
  if (!id)
    return res
      .status(500)
      .json({ err: true, message: 'required parameter was not passed: id' });
  if (!cart)
    return res
      .status(500)
      .json({ err: true, message: 'required parameter was not passed: cart' });
...
router.post('/payment', async (req, res) => {
  ...
  if (!hash) {
    res
      .status(500)
      .json({ err: true, message: 'required parameter was not passed: hash' });
    return;
  }

  const paymentResult = await ps.checkPayment();
